...
func (c *comparisonListServiceImplementation) AddInstrument(id uint64, instrumentId uint64) error {
	c.logger.WithFields(map[string]interface{}{"comparisonList_id": id, "instrument_id": instrumentId}).Info("comparisonList add instrument called")

	_, err := c.comparisonListRepository.Get(id)

	if err != nil && err == repositoryErrors.ObjectDoesNotExists {
		c.logger.WithFields(map[string]interface{}{"comparisonList_id": id, "instrument_id": instrumentId}).Error(serviceErrors.ComparisonListAddInstrumentFailed.Error() + serviceErrors.ComparisonListDoesNotExists.Error())
		return serviceErrors.ComparisonListDoesNotExists
	} else if err != nil {
		c.logger.WithFields(map[string]interface{}{"comparisonList_id": id, "instrument_id": instrumentId}).Error(serviceErrors.ComparisonListAddInstrumentFailed.Error() + err.Error())
		return err
	}

	_, err = c.instrumentRepository.Get(instrumentId)
	if err != nil && err == repositoryErrors.ObjectDoesNotExists {
		c.logger.WithFields(map[string]interface{}{"comparisonList_id": id, "instrument_id": instrumentId}).Error(serviceErrors.ComparisonListAddInstrumentFailed.Error() + serviceErrors.InstrumentDoesNotExists.Error())
...
func (c *comparisonListServiceImplementation) DeleteInstrument(id uint64, instrumentId uint64) error {
	c.logger.WithFields(map[string]interface{}{"comparisonList_id": id, "instrument_id": instrumentId}).Info("comparisonList delete instrument called")
